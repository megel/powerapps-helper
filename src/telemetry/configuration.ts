...
export function createTelemetryReporter(productName: string, context: ExtensionContext, key: string, sessionId: string): TelemetryReporter {
    const telemetry = new TelemetryReporter(EXTENSION_ID, vscode.extensions.getExtension(EXTENSION_ID)?.packageJSON.version, key);
    ...
    if (client) {
        ...
        const productVersion: string = vscode.extensions.getExtension(EXTENSION_ID)?.packageJSON.version;
